...
export function DayCard({ day, dayOrdinalLabel, defaultOpen = false }: Props) {
  ...
  return (
    <div
      data-day="card"
      className="bg-white border border-[#E2DDD5] rounded-2xl overflow-hidden shadow-[0_1px_3px_rgba(15,58,51,.06),0_1px_2px_rgba(0,0,0,.04)] hover:shadow-[0_4px_16px_rgba(15,58,51,.08),0_1px_3px_rgba(0,0,0,.05)] transition-shadow"
    >
      {/* ── Header ── */}
      <button
        data-day="header"
      ...
      {/* ── Body (forced open in print via CSS) ── */}
      <div
        data-day="body"
        ...
        <div className="px-6 pb-2">
          {day.items.map((item, i) => (
            ...
            >
              ...
              {/* Content */}
              <div>
                <div data-day="item-title" className="text-[14px] font-medium text-[#1A1A1A] mb-1 leading-[1.4]">
                  {item.title}
                </div>
                {item.description && (
                  <div data-day="item-desc" className="text-[12.5px] font-light text-[#4A4A4A] leading-[1.6]">
                    {item.description}
                  </div>
                )}
