 ...
 *  1. All Link hrefs use internal paths (/planner, /guides, /about) — next-intl
 *     resolves to localized URLs. No hardcoded /guias, /trip-generator, etc.
 *  2. Guide cards use getGuideUrl() from lib/routes.ts for server-side href generation.
 *     Each guide now carries slug_es + slug_en.
 *  3. Metadata uses buildAlternates() from lib/seo.ts.
 *  4. Colors reference new brand tokens (Pine, Sand, Sage, Coral).
 *  5. font-playfair classes replaced with font-display (Fraunces).
 *     font-sans stays as Manrope (updated in layout.tsx).
 *  6. Hero H1 uses Fraunces italic for the emphasis line ("Plan less.")
 *     as per brand guide: Fraunces = hero emphasis.
 */
...
// ── Guide data ─────────────────────────────────────────────
// Single source of truth lives in lib/guides.ts — no duplication.
const GUIDES = getAllGuides()

const REVIEWS = [
  {
    quote_es: 'Soltamos el teléfono para conectar con lo real. Caminatas, helado artesanal y comida deliciosa. Un tiempo de calidad donde todo se sintió en su medida justa. Volvimos renovados.',
    quote_en: 'We put down our phones to connect with what was real. Hikes, artisanal ice cream, amazing food. Quality time where everything felt just right.',
    name:     'Jorge',
    img:      '/images/reviews/jorge.png',
  },
  {
    quote_es: 'Buen vino y arquitectura con historia. Encontramos el punto medio entre aventura y descanso total. Una experiencia curada con un gusto impecable.',
    quote_en: 'Good wine and architecture with history. We found the sweet spot between adventure and total rest. An experience curated with impeccable taste.',
    name:     'Julia',
    img:      '/images/reviews/julia.png',
  },
  {
    quote_es: 'La plataforma es súper intuitiva; en tres clics pasamos de la idea al itinerario perfecto. Fue esencial para organizar nuestro viaje con total claridad y calma.',
    quote_en: 'The platform is incredibly intuitive — three clicks and we went from idea to the perfect itinerary. Essential for organizing our trip with total clarity and calm.',
    name:     'Ana',
...
const FEATURE_LABELS: Record<string, { title_es: string; title_en: string; body_es: string; body_en: string }> = {
  curated:      { title_es: 'Guías curadas',         title_en: 'Curated guides',       body_es: 'Ideas claras para decidir rápido: qué vale la pena, qué saltarte y cómo vivir el destino con buen ritmo.',             body_en: 'Clear ideas to decide fast: what\'s worth it, what to skip, and how to experience the destination at the right pace.' },
  itineraries:  { title_es: 'Itinerarios simples',   title_en: 'Simple itineraries',   body_es: 'Pasa de inspiración a un plan por día en minutos. Sin Excel, sin mil tabs.',                                           body_en: 'Go from inspiration to a day-by-day plan in minutes. No Excel, no endless tabs.' },
  personalized: { title_es: 'Personalizado',         title_en: 'Personalized',         body_es: 'Familias, parejas, relax o aventura: sugerencias que se adaptan a tu forma de viajar.',                                  body_en: 'Families, couples, relaxed or adventurous: suggestions that adapt to your travel style.' },
  clarity:      { title_es: 'Claridad total',        title_en: 'Total clarity',        body_es: 'Recomendaciones concretas y ordenadas para que planificar se sienta ligero.',                                            body_en: 'Concrete, organized recommendations so planning feels effortless.' },
}
...
export default async function HomePage({
  ...
  return (
    <main>
          ...
          <div className="grid md:grid-cols-2 gap-14 items-start py-[72px] max-[768px]:grid-cols-1 max-[768px]:py-10 max-[768px]:gap-10">
            ...
            <div>
              <span className="sec-label">
                {isES ? 'Planificador de viajes con IA' : 'AI-powered travel planner'}
              </span>
              <h1 className="font-sans text-[60px] max-[768px]:text-[40px] font-bold text-[#0F3A33] leading-[1.05] tracking-[-1.5px] mb-5">
                {isES ? 'Tu próxima aventura comienza aquí' : 'Your next adventure starts here'}
              </h1>
              ...
              <div className="flex items-center gap-3">
                <div className="flex">
                  {REVIEWS.map(r => (
                    ...
                    >
                      <Image src={r.img} alt={r.name} width={30} height={30} className="w-full h-full object-cover" />
                    </div>
                  ))}
                </div>
                <p className="font-sans text-[11px] text-[#3E5F58] tracking-[.2px] ml-3">
                  ...
                  <br />
                  <span className="text-[#E1615B] tracking-[2.5px]">★★★★★</span>
                </p>
              </div>
            </div>
      ...
      {/* ② FEATURES */}
      <section className="py-24 max-[768px]:py-16" style={{ background: '#FFFFFF', borderTop: '1px solid rgba(200,191,181,.3)' }}>
        <div className="page-inner">
          <span className="sec-label">
          ...
          <h2 className="font-sans text-[40px] max-[768px]:text-[30px] font-bold text-[#0F3A33] leading-[1.15] mb-4">
            {isES ? 'Tu guía y tu plan en una sola vista.' : 'Your guide and plan in one view.'}
          </h2>
  ...
  {f.key === 'clarity' && (
    <FeatureIcon>
      ...
      {/* ③ PHILOSOPHY */}
      <section className="py-24 max-[768px]:py-16" style={{ background: '#FFF9F3', borderTop: '1px solid rgba(200,191,181,.3)' }}>
        <div className="page-inner">
          <div className="grid md:grid-cols-2 gap-20 items-center max-[768px]:grid-cols-1 max-[768px]:gap-9">
            <div>
              ...
              <div className="flex gap-3.5 mt-8 flex-wrap">
                {/* Coral CTA — brand guide primary action color */}
  ...
  <div className="w-full max-w-[420px]">
    ...
    />
      ...
      <section className="py-24 max-[768px]:py-16" style={{ background: '#FFFFFF', borderTop: '1px solid rgba(200,191,181,.3)' }}>
        <div className="page-inner">
          <div className="flex items-end justify-between mb-10 max-[768px]:flex-col max-[768px]:items-start max-[768px]:gap-3">
            <div>
              <span className="sec-label">{isES ? 'Guías curadas' : 'Curated guides'}</span>
      ...
      <section
        ...
        style={{ background: '#FFF9F3', borderTop: '1px solid rgba(200,191,181,.3)' }}
      >
        <div className="page-inner">
      ...
      <section
        ...
        style={{ background: '#FFFfFf', borderTop: '1px solid rgba(200,191,181,.3)' }}
      >
        {/* CDMX skyline watermark */}
        <svg className="absolute bottom-0 left-0 right-0 h-[80px] opacity-[0.05] pointer-events-none" viewBox="0 0 1200 80" preserveAspectRatio="xMidYMax meet" aria-hidden="true">
          <rect x="20" y="15" width="44" height="65" fill="#0F3A33"/><rect x="75" y="35" width="30" height="45" fill="#0F3A33"/><rect x="125" y="5" width="54" height="75" fill="#0F3A33"/><rect x="190" y="28" width="38" height="52" fill="#0F3A33"/><rect x="250" y="18" width="46" height="62" fill="#0F3A33"/><rect x="315" y="2" width="58" height="78" fill="#0F3A33"/><rect x="695" y="0" width="63" height="80" fill="#0F3A33"/><rect x="875" y="8" width="53" height="72" fill="#0F3A33"/>
        </svg>
        <div className="page-inner relative z-10">
          ...
          <div className="grid grid-cols-3 gap-6 max-[768px]:grid-cols-1 max-[1024px]:grid-cols-2">
            {REVIEWS.map(r => (
              <div key={r.name} className="rounded-[12px] p-9 max-[768px]:p-7" style={{ background: '#FFF9F3', border: '1px solid rgba(200,191,181,.22)' }}>
                <p className="rounded-[12px] p-7 max-[768px]:p-6 ...">
                  &ldquo;{isES ? r.quote_es : r.quote_en}&rdquo;
                </p>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full overflow-hidden flex-shrink-0">
                    <Image src={r.img} alt={r.name} width={36} height={36} className="w-full h-full object-cover" />
                  </div>
                  <div>
                    <p className="font-sans text-[13px] font-semibold text-[#0F3A33]">{r.name}</p>
                    <p className="font-sans text-[10px] tracking-[2px] text-[#E1615B]">★★★★★</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      >
        <div className="page-inner">
          ...
          <div className="flex gap-3.5 justify-center flex-wrap max-[768px]:flex-col max-[768px]:items-center">
            <Link href="/planner" className="btn-solid max-[768px]:w-full max-[768px]:text-center">
              {isES ? 'Planificar' : 'Plan my trip'}
            </Link>
            <Link href="/guides" className="btn-outline max-[768px]:w-full max-[768px]:text-center">
